...
folder_path = '../5_dataset'
trajectories = load_data_from_folder(folder_path)

# Split data into training and testing sets
train_trajectories, test_trajectories = train_test_split(trajectories, test_size=0.2, random_state=42)
...
training_pairs = create_training_pairs(train_trajectories)
testing_pairs = create_training_pairs(test_trajectories)
...
def train_model(model, criterion, optimizer, training_pairs, testing_pairs, num_epochs=100, batch_size=32):
    model.train()
    epoch_losses = []
    test_losses = []
    ...
    for epoch in range(num_epochs):
        ...
        test_loss = 0
        with torch.no_grad():
            for i in range(0, len(testing_pairs), batch_size):
                batch_pairs = testing_pairs[i:i + batch_size]
                if len(batch_pairs) == 0:
                    continue
        ...
        test_loss_avg = test_loss / len(testing_pairs)
        test_losses.append(test_loss_avg)
        print(f'Epoch [{epoch + 1}/{num_epochs}], Testing Loss: {test_loss_avg:.4f}')
        model.train()
...
# Train the model
epoch_losses, test_losses = train_model(model, criterion, optimizer, training_pairs, testing_pairs, num_epochs)
