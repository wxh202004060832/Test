...
def save_trajectory_to_csv(actual_points, predicted_points, file_path):
    actual_points = np.array(actual_points)
    predicted_points = np.array(predicted_points)

    # 创建一个数据框，其中包括实际轨迹和生成的轨迹
    df = pd.DataFrame({
        'Actual_X': actual_points[:, 0],
        'Actual_Y': actual_points[:, 1],
        'Actual_Z': actual_points[:, 2],
        'Predicted_X': np.nan,
        'Predicted_Y': np.nan,
        'Predicted_Z': np.nan
    })
    ...
    for i, point in enumerate(predicted_points):
        if i < len(df):
            df.loc[i, ['Predicted_X', 'Predicted_Y', 'Predicted_Z']] = point
        else:
            df = df.append(pd.Series([np.nan, np.nan, np.nan, *point], index=df.columns), ignore_index=True)

    df.to_csv(file_path + '_trajectory.csv', index=False)
...
def main():
    ...
    # Load new data
    new_file_path = '../7_dataset_new/CCX8_sampled_5_filtered.csv'
    ...
    # Save the trajectories to a single CSV file
    save_trajectory_to_csv(traj, predicted_points, 'predicted_new')
