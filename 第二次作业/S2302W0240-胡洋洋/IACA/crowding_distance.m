function [Population_cd]=crowding_distance(Population_ns,aim,last_rank)
%求个体的拥挤距离
...
s=size(vol,2);
obj_array=zeros(s,aim);
Pop_struct(1:s)=struct('Chromesome',[],'decode',[],'machine_start_time',[],'machine_end_time',[],'factory_bj',[],'worker_bj',[],'worker_protime',[],'objectives',[],'IT',[],'rank',0,'crowded_distance',0); 
for j=1:s
    obj_array(j,:)=Pop_array1(j).objectives;                               %取出对应个体的目标值
end
for i=1:aim                                                                  % 分别以m个目标值计算拥挤距离，并将其加在一起
%% 对选定的非支配前沿的个体进行排序
