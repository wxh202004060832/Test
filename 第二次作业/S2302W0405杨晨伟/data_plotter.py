...
class Plotter:
    ...
    def show_time_series(self, start_year: int, end_year: int,  intermediate=False):
        self.__validate_dates(start_year=start_year, end_year=end_year)
        ...
        for year in range(start_year, end_year + 2):
            if year == end_year + 1:
                ...
                if intermediate:
                    print(
                        f"\nShowing intermediate years' plots for {self.company_name} from {start_year} to {end_year}:")
                    for year in range(start_year, end_year + 1):
                        if year in self.__company_handler.years:
                            self.show_yearly_prices(year=year)
                            print(
                                f"\nShowing intermediate years' plots for {self.company_name} from {start_year} to {end_year}:")
            else:
