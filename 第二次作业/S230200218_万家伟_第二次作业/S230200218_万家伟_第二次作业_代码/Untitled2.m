...
[m,n]= size(train_x1);
%-----------定义模型
generator=nnsetup([1,15,1]);
discriminator=nnsetup([1,15,1]);
%----------参数设置
batch_size= m; 
iteration= 10000;
images_num= m;
...
for i = 1: iteration
    ...
    images_real = train_x1;
    noise = unifrnd(0,1,m,1);
    %开始训练
    %--------更新生成器，固定住辨别器
    ...
    generator = nnff(generator, noise);
    images_fake = generator.layers{generator.layers_count}.a;
    images1 = [images_fake; images_real]; 
    discriminator = nnff(discriminator, images1);
...
for i = 1: iteration
    ...
    generator = nnff(generator, noise);
    images_fake = generator.layers{generator.layers_count}.a;
    images2 = [images_fake; images_real]; 
    discriminator = nnff(discriminator, images2);
    logits = discriminator.layers{discriminator.layers_count}.z;
    labels = [zeros(batch_size,1); ones(batch_size,1)];
    discriminator = nnbp_d(discriminator, logits, labels);
    discriminator = nnapplygrade(discriminator, learning_rate);
...
for i = 1: iteration
    ...
    generator = nnff(generator, noise);
    images_fake = generator.layers{generator.layers_count}.a;
    images3 = [images_fake; images_real]; 
    discriminator = nnff(discriminator, images3);
    logits = discriminator.layers{discriminator.layers_count}.z;
    labels = [zeros(batch_size,1); ones(batch_size,1)];
    discriminator = nnbp_d(discriminator, logits, labels);
    discriminator = nnapplygrade(discriminator, learning_rate);
...
for i = 1: iteration
    ...
    generator = nnff(generator, noise);
    images_fake = generator.layers{generator.layers_count}.a;
    images4 = [images_fake; images_real]; 
    discriminator = nnff(discriminator, images4);
    logits = discriminator.layers{discriminator.layers_count}.z;
    labels = [zeros(batch_size,1); ones(batch_size,1)];
    discriminator = nnbp_d(discriminator, logits, labels);
    discriminator = nnapplygrade(discriminator, learning_rate);
...
for i = 1: iteration
    ...
    generator = nnff(generator, noise);
    images_fake = generator.layers{generator.layers_count}.a;
    images5 = [images_fake; images_real]; 
    discriminator = nnff(discriminator, images5);
    logits = discriminator.layers{discriminator.layers_count}.z;
    labels = [zeros(batch_size,1); ones(batch_size,1)];
    discriminator = nnbp_d(discriminator, logits, labels);
    discriminator = nnapplygrade(discriminator, learning_rate);
