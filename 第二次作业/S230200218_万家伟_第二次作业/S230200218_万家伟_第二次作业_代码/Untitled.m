...
%% BiLSTM
inpupts=[images_real;images_real;images_real;images_real]
demands=inpupts(1:342,5);
orgin=inpupts(1:342,1:4);
in=orgin;
out=demands;
i=1;
while ~isempty(in)
...
%% 测试数据处理
clear X Y
demands=inpupts(342:360,5);
orgin=inpupts(342:360,1:4);
in=orgin;
out=demands;
i=1;
while ~isempty(in)
...
testYreal1=testYreal(1:16,1);
Yreal1=Yreal(1,1:16);
figure, ploterrhist(testYreal1-Yreal1'), title('test')
figure, plot(testYreal1,'-o')
hold on
plot(Yreal1,'-^')
title('微磨具磨损预测结果')
xlabel('数据组数')
...
legend('BiLSTM模型预测值','实际磨损值','Location','NorthWest')
%% 计算误差
RMSE=sqrt(mean((testYreal1-Yreal1').^2))
R=corrcoef(testYreal1,Yreal1')
r=R(1,2)
disp(['均方根误差',num2str(RMSE)])
disp(['决定系数R^2为',num2str(r)])
error = abs(Yreal1' - testYreal1)./testYreal1;
result = [testYreal1 Yreal1' error]
%%
% sigmoid函数
