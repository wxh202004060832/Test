...
# 截取上面数据集中的前21列，包括所有20列特征和一列输出
data = DATA.iloc[0:243, 0:6]#0:13

# 将读取的数据转换为二维数组矩阵类型，57*21。data_array是全部数据
data_array = data.values

# 截取20个特征并转换成二维数组矩阵形式
data_input = data.iloc[0:243, 0:5]  #0:32, :11
data_input_array = data_input.values

# 截取一列输出并转换成二维数组矩阵形式
data_output = data.iloc[0:243, 5:6]  #0:32, 12:13
...
#'max_depth': 14.158760498274312, 'max_features': 5.642782028951499, 'min_samples_split': 2.0, 'n_estimators': 179.86454038632178
# 6.2、BO_SVR
...
list_Predspear=[]
# 遍历各模型
for model_mot in models:
    # 在训练集上训练
    model_mot.fit(x_train, y_train)
    # 模型预测结果
    pred = model_mot.predict(x_test)
    pred=pred.reshape(-1,1)

    # 将各模型预测结果保存到相应的数组中
    MAE = mean_absolute_error(y_test, ss_y.inverse_transform(pred))
    r2=r2_score(y_test, ss_y.inverse_transform(pred))
    e=y_test-ss_y.inverse_transform(pred)
    prde_spear=stats.spearmanr(ss_y.inverse_transform(pred),y_test)
    RMSE=np.sqrt(mean_squared_error(y_test, ss_y.inverse_transform(pred)))
    list_MAE.append(MAE)
    list_Prediction.append(ss_y.inverse_transform(pred))
    list_e.append(e)
