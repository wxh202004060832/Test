function rs = regrrbf2(y,x,eg,tf,varargin)
...
switch(tf)
  ...
  case 'multiquadric'
    sp = var{1};
    b = sqrt(-log(.5))/sp;
    P = sqrt(1+(dist(p',p)*b).^2);  
  case 'inversemultiquadric'
    sp = var{1};
    b = sqrt(-log(.5))/sp;
    P = 1./sqrt(1+(dist(p',p)*b).^2); 
  otherwise
...
switch(tf)
  ...
  case 'thinplate'
    ...
    %a1=z;
  case 'multiquadric'
    a1 = sqrt(1+(dist(w1,p)*b).^2); 
  case 'inversemultiquadric' 
    a1 = 1./sqrt(1+(dist(w1,p)*b).^2); 
  otherwise
    disp('Type not in list!');
    w1 = NaN;
    return;
...
switch(tf)
  ...
  case 'multiquadric'
    sp = var{1};
    b = sqrt(-log(.5))/sp;
    P = sqrt(1+(dist(xc,x')*b).^2);  
    xx = [ones(1,size(x,1));P]';
  case 'inversemultiquadric'
    sp = var{1};
    b = sqrt(-log(.5))/sp;
    P = 1./sqrt(1+(dist(xc,x')*b).^2);  
    xx = [ones(1,size(x,1));P]';  
  otherwise
    disp('Type not in list!');
    xx = NaN;
